...
/*----------------------------------------------------------------------
 ...
 +---------------------------------------------------------------------*/
PLT_MicroMediaController::PLT_MicroMediaController(PLT_CtrlPointReference& ctrlPoint,PltMediaInterface * delegateWrapper) :
...
{
    NSMutableArray<EventParamsResponse *> *stateArray = [NSMutableArray array];
    ...
    while (var) {
        ...
        EventParamsResponse * state = [[EventParamsResponse alloc] initWithDeviceUUID:deviceUUID ServiceID:serviceID EventName:eventName EventValue:eventValue];
...
PLT_MediaObject* PLT_MicroMediaController::create_didl_from_path(NPT_String	path)
{
    NPT_String              uri;
    ...
    printf("Building didl for file '%s'", (const char*)path);
    ...
    if(object){
        ...
        object->m_Title =  NPT_FilePath::BaseName(path, false);
        if(object->m_Title.GetLength()==0){
            
        }
        
        /* Set the protocol Info from the extension */
        resource.m_ProtocolInfo = PLT_ProtocolInfo::GetProtocolInfo(path, true,NULL);
        if (!resource.m_ProtocolInfo.IsValid())  goto failure;
        
        
        object->m_ObjectClass.type = PLT_MediaItem::GetUPnPClass(path, NULL);
...
PLT_MicroMediaController::setRendererAVTransportURI(const char *uriStr, const char *metaData)
{
    PLT_DeviceDataReference device;
    GetCurMediaRenderer(device);
    NPT_String meta_didl = "";
    PLT_MediaObjectReference item;
    NPT_String tmp_meta = metaData;
    ...
    if (!device.IsNull()) {
        ...
        {
            //构造
            item = create_didl_from_path(uriStr);
            NPT_String tmp;
            PLT_Didl::ToDidl(*item.AsPointer(),"",tmp);
            meta_didl = didl_header +tmp + didl_footer;
            bool rel = NPT_FAILED(SetAVTransportURI(device, 0, uriStr, meta_didl,NULL));
            if(rel)
            {
                                   ...
                                   void*                    userdata)
{
    ...
    if(m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(getCurrentAVTransportActionResponse:)])
    {
        ...
        CurrentAVTransportActionResponse *capsule = [[CurrentAVTransportActionResponse alloc]initWithResult:result DeviceUUID:deviceUUID Actions:actionArr UserData: (__bridge id)userdata];
...
OnGetPositionInfoResult(
                        ...
                        void*                    userdata)
{
    if(m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(getPositionInfoResponse:)])
    {
        NSInteger result = res;
        NSString *deviceUUID = [NSString stringWithUTF8String:device->GetUUID()];
        CurentPositionInfoResponse *capsule = [[CurentPositionInfoResponse alloc]initWithResult:result DeviceUUID:deviceUUID UserData:(__bridge id)userdata];
        
        
        capsule.cur_postion = [NSString stringWithUTF8String:info->track_uri];
        
        capsule.duration = info->track_duration.ToSeconds();
        capsule.postion = info->rel_time.ToSeconds();
        
        //capsule.cur_duration =info->track_duration.ToSeconds();
        ...
        [m_Target.delegate getPositionInfoResponse:capsule];
                         ...
                         void*                    userdata)
{
    if(m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(getTransportInfoResponse:)])
    {
        ...
        TransportInfoResponse *capsule = [[TransportInfoResponse alloc]initWithResult:result DeviceUUID:deviceUUID UserData: (__bridge id)userdata];
             ...
             void*                    userdata)
{
    if(m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(nextResponse:)])
    {
        ...
        EventResultResponse *capsule = [[EventResultResponse alloc]initWithResult:result DeviceUUID:deviceUUID UserData: (__bridge id)userdata];
                  ...
                  void*                    userdata)
{
    if(m_Target.delegate && [m_Target.delegate respondsToSelector:@selector(getVolumeResponse:)])
    {
        ...
        VolumResponse * capsule = [[VolumResponse alloc] initWithResult:result DeviceUUID:deviceUUID UserData:(__bridge id)userdata Channel:cnl Volume:volume];
